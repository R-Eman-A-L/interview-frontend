<script lang="ts">
	...
	let difficulty: 'chill' | 'hard' = 'chill';
	...
	async function onCreateInterview(event: SubmitEvent) {
		...
		try {
			...
			await goto(`/interview/${res.interview_id}/ingest?mode=${difficulty}`);
		} catch (e: any) {
...
<form on:submit={onCreateInterview} style="display:grid; gap:12px; max-width:520px;">
	...
	<label>
		Role:
		<input type="text" bind:value={role} placeholder="Android Developer" required />
	</label>
